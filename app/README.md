...
│   │   │   └── extract/    # Screenshot parsing endpoint
│   │   └── auth/
│   ├── login/
│   └── register/
├── components/
│   ├── ai/                 # Chat window, screenshot upload
│   ├── dashboard/          # Stat cards, cashflow chart
...
**Double-entry in the backend, single-entry in the UI** — users fill a simple form; the system generates the correct journal entry pairs automatically. The balance sheet always balances without the user needing to know what debit and credit mean.
...
**AI never auto-commits** — all AI-parsed results are `DRAFT` status until the user explicitly confirms. This is a non-negotiable principle for financial data integrity.
