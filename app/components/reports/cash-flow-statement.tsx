...
export function CashFlowStatement({ data }: { data: CashFlowData }) {
  ...
  return (
    <div>
      <ReportTable title="Operating Activities" rows={data.operating} totalRow={{ label: "Net from Operations", monthly: sectionTotal(data.operating) }} highlightTotal />
      <ReportTable title="Investing Activities" rows={data.investing} totalRow={{ label: "Net from Investing", monthly: sectionTotal(data.investing) }} highlightTotal />
      <ReportTable title="Financing Activities" rows={data.financing} totalRow={{ label: "Net from Financing", monthly: sectionTotal(data.financing) }} highlightTotal />

      <div className="overflow-x-auto rounded-lg border-2 border-gray-200 dark:border-slate-600 bg-gray-50 dark:bg-slate-900/50 mt-2">
          ...
          <tbody>
            <tr>
              ...
              <td className={cn("px-3 py-3 text-right font-bold text-sm min-w-[6rem]",
                data.netByMonth.total > 0 ? "text-green-600 dark:text-green-400"
                : data.netByMonth.total < 0 ? "text-red-500 dark:text-red-400"
